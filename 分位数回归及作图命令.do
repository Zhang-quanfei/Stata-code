...
xtqreg cipin dtmd lasset lyysr net_syl cwgg merge own_con rkmd lgdp i.year, i(sid) quantile(.05(0.1)0.95)  //面板分位数回归


local x1 " dtmd lasset lyysr net_syl cwgg merge own_con rkmd lgdp"   //局部宏定义所有解释变量
*list  `x1'   //列出所有被解释变量
...
mat list result1
matselrc result1 cresult, c(2/21) //19列 = 2*分位个数+1列无用项（第一列），保留有用数据列
mat list cresult
...
preserve 
keep reg1 reg3 reg5 reg7 reg9 reg11 reg13 reg15 reg17  reg19  //保留系数列
rename (reg1 reg3 reg5 reg7 reg9 reg11 reg13 reg15 reg17  reg19) (reg1 reg2 reg3 reg4 reg5 reg6 reg7 reg8 reg9 reg10)   //重命名
xpose,clear  //转置数据
...
matrix input myrvec = (0.05,0.15,0.25,0.35,0.45,0.55,0.65,0.75,0.85,0.95)
mat list myrvec
...
forv i = 1/1{
twoway (line up_v`i' qqp,lc(black*1.4) lpattern(dash) xlabel(0.05 0.15 0.25 0.35 0.45 0.55 0.65 0.75 0.85 0.95)) ///
(line low_v`i' qqp,lc(black*1.4) lpattern(dash) xlabel(0.05 0.15 0.25 0.35 0.45 0.55 0.65 0.75 0.85 0.95))  ///
